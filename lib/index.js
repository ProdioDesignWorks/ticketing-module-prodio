...
var getTicket = function getTicket(payload, BASE_URL, callback) {
  if (!isJson(payload)) {
    ...
    if (!isJson(payload)) {
      ...
      var url = BASE_URL + '/tickets/details?ticketId=' + payload.ticketId;
      axios.get(url, payload).then(function (response) {
        return callback(response);
      }).catch(function (error) {
        var json = stringify(error);
...
var getAllTickets = function getAllTickets(payload, BASE_URL, callback) {
  if (!isJson(payload)) {
    ...
    if (!isJson(payload)) {
      ...
      var url = BASE_URL + '/tickets/list?status=' + payload.status + '&number=' + payload.number + '&from=' + payload.from + '&to=' + payload.to + '&title=' + payload.title + '&assignee=' + payload.assignee;
      axios.get(url, payload).then(function (response) {
        return callback(response);
      }).catch(function (error) {
        var json = stringify(error);
